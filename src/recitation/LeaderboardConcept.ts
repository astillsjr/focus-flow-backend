...
export default class LeaderboardConcept {
  ...
  async createPlayer(
    ...
    const query = { name };
    const cursor = await this.players.findOne(query);
    if (cursor) return { error: "Player with this name already exists" };
    ...
    await this.players.insertOne(newPlayer);
  ...
  async submitScore(
    ...
    const query = { _id: player }
    const cursor = await this.players.findOne(query);
    if (!cursor) return { error: "Player not found" };

    const newScoreId = freshID();
  ...
  async _getPlayerScores({ player }: { player: Player }): Promise<ScoreDoc[]> {
    ...
    const query = { player };
    const cursor = this.scores.find(query);
  ...
  async _getPlayerStats(
    ...
    const cursor = this.scores.aggregate(pipeline);
    
    const results = await cursor.toArray();
    
    if (results.length === 0) return null;

    const { totalScores, highestScore, averageScore } = results[0];
    return { totalScores, highestScore, averageScore };
  }
}
